...
// --- Robust runtime guards (logs only; no behavior change) ---
...
spawn() {
  // reset manual flag on explicit spawns
  this.manual = false;
  ...
  if (!host || host === 'play.server.com') {
    this.log('error', 'Invalid server host. Set a real server in Settings → Server IP.');
    return;
  }
  ...
  // schedule create with optional login delay, but only after DNS resolves
  const schedule = () => {
  ...
  dns.lookup(host).then(() => schedule()).catch((err) => {
    ...
    this.manual = true; // block auto-reconnect loops
  });
}
  }
  ...
  scheduleReconnect() {
    ...
    this.reconnectTimer = setTimeout(() => this.spawn(), backoff);
  }
...
const manager = new BotManager((msg) => {
  ...
  wss.clients.forEach((ws) => { if (ws.readyState !== ws.OPEN) return; const id = msg && msg.accountId; if (!id || ws.subs?.has('ALL') || (id && ws.subs?.has(id))) safeSend(ws, msg); });
});

wss.on('connection', (ws, req) => {
  ws.subs = new Set(); // per-client subscriptions
