(function(){
  ...
  function sendSubscription() {
    // if no online accounts, unsubscribe from all to avoid stray logs
    const onlineIds = Array.from(accounts.values()).filter(a => a.status === 'online').map(a => a.id);
    if (onlineIds.length === 0) {
      ws.send(JSON.stringify({ type:'subscribe', accounts: [] }));
      return;
    }
    if (selectedAccount === 'all') {
      ws.send(JSON.stringify({ type:'subscribe', all: true }));
    } else {
      ws.send(JSON.stringify({ type:'subscribe', accounts: [selectedAccount] }));
    }
  }
...
shardsAccountSel?.addEventListener('change', startShardsLoop);
  ...
  ws.addEventListener('open', () => { log('Connected to backend'); sendSubscription(); startShardsLoop(); });
