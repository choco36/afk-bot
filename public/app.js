(function(){
  ...
  // === persistence ===
  const STORAGE_KEY = 'afk_accounts_v1';
  function saveAccounts(){
    try{
      const arr = Array.from(accounts.values()).map(a => ({ id:a.id, label:a.label, mode:a.mode }));
  ...
  function loadAccounts(){
    try{
      ...
      arr.forEach(a => { accounts.set(a.id, { id:a.id, label:a.label, mode:a.mode, status:'offline' }); });
    }catch(e){ /* ignore */ }
  }
...
function pingShardsOnce(){
  const id = chooseShardsAccount();
  if (!id) return;
  try { ws.send(JSON.stringify({ type:'chat', accountId:id, text:'/shards' })); } catch {}
}
...
shardsAccountSel?.addEventListener('change', startShardsLoop);
  ...
  ws.addEventListener('open', () => { log('Connected to backend'); try{ ws.send(JSON.stringify({type:'list'})); }catch{} startShardsLoop(); pingShardsOnce(); });
  ws.addEventListener('message', (e) => {
    ...
    if (msg.type === 'state') {
      // Update statuses and backfill unknown accounts so the user can control active sessions after reload
      (msg.items || []).forEach(s => {
        if (!accounts.has(s.id)) {
          accounts.set(s.id, { id:s.id, label:'#'+String(s.id).slice(0,6), mode:(s.mode||'microsoft'), status: s.connected?'online':'offline' });
        } else {
          accounts.get(s.id).status = s.connected ? 'online' : 'offline';
        }
      });
      refreshShardsDropdown();
      updateAccountsUI();
      // Immediate shards ping on fresh state to populate count quickly
      pingShardsOnce();
      return;
    }
    if (msg.type === 'spawned') {
