...
function setLockUI(locked){
  $('#lockDot').className = 'w-3 h-3 rounded-full ' + (locked ? 'bg-red-500' : 'bg-green-500')
  $('#lockText').textContent = locked ? 'מצב: נעול – צריך להזין טוקן' : 'מצב: פתוח – מאומת'
}
...
async function checkLock(){ await setServerCookieFromToken(); try{ const r=await fetch(withTokenUrl('/api/me'),{headers:authHeaders()}); const d=await r.json(); setLockUI(!d.authorized) }catch{ setLockUI(true) } }
...
// Bootstrap: token from URL
const urlTok = new URLSearchParams(location.search).get('token')
if(urlTok){ localStorage.setItem(tokenKey, urlTok.trim()) }
checkLock()
...
function renderSessions(list){
  ...
  if(!list.length){
    sessionsBox.innerHTML='<div class="text-sm text-neutral-500">אין סשנים</div>'
    chatSel.innerHTML='<option value=\"\">בחר סשן…</option>'
    return
  }
  list.forEach(s=>{
...
function appendChat(line){
  chatLines.push(line)
  if(chatLines.length>500) chatLines.splice(0,chatLines.length-500)
  ...
  ws.onmessage=(ev)=>{
    try{
      ...
      if(type==='sessions'){ renderSessions(payload) }
        ...
        if(payload.entry.level==='auth'){
          const {url,code}=parseAuthMsg(payload.entry.msg)
          showAuthBanner(payload.entry.msg); showMsaModal(url,code); try{ window.open(url,'_blank','noopener') }catch{}
        }
        if(['chat','you','ok','info','warn','error','trace'].includes(payload.entry.level)){ appendChat(payload.entry) }
      }
    }catch{}
  }
}catch{}
