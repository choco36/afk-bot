(function(){
  ...
  function log(line, level='info') {
    ...
    consoleEl.textContent += `[${new Date().toLocaleTimeString()}] ${prefix} ${line}\n`;
    consoleEl.scrollTop = consoleEl.scrollHeight;
  }

  ws.addEventListener('open', () => log('Connected to backend'));
  ws.addEventListener('message', (e) => {
    ...
    if (msg.type === 'chat') {
      if ($('#showChat').checked && (selectedAccount === 'all' || selectedAccount === msg.accountId)) {
        log(`[CHAT][#${(msg.accountId||'').slice(0,6)}] ${msg.text}`);
      }
    }
