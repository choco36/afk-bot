...
class BotManager {
  ...
  _anti(s){
    ...
    const mode=c.afkMode||'jitter'
    const iv = Math.max(15000, Number(c.afkIntervalMs||60000))
    ...
    s.afkTimer = setInterval(()=>{
      try{
        ...
        if(mode==='jitter'){
          b.setControlState('jump',true); setTimeout(()=>b.setControlState('jump',false),250)
        } else if(mode==='circle'){
          b.setControlState('forward',true); setTimeout(()=>b.setControlState('forward',false),1200)
        } else if(mode==='strafe'){
          const side=Math.random()<.5?'left':'right'; b.setControlState(side,true); setTimeout(()=>b.setControlState(side,false),700)
        } else if(mode==='walkabout'){
          const dir=Math.random()<.5?'forward':'back'; b.setControlState(dir,true); setTimeout(()=>b.setControlState(dir,false),700)
        }
        this.log(s.id,'trace','Anti-AFK '+mode)
      }catch(e){
        this.log(s.id,'warn','Anti-AFK error: '+(e?.message||e))
      }
    }, iv)
  }
  async start(cfg){
    const id = uuidv4()
    const s = { id, cfg, createdAt:Date.now(), status:'starting', logs:[], bot:null, afkTimer:null, reTimer:null }
    this.sessions.set(id,s)
    this.push('sessions', this.list())
    ...
    const go = () => {
      ...
      s.bot = bot
      const set=(st)=>{ s.status=st; this.push('sessions',this.list()) }

      bot.once('login', ()=>{
        set('online')
        this.log(id,'ok','Logged as '+bot.username)
        if(cfg.loginOnly){ setTimeout(()=>{ try{ bot.quit('login-only') }catch{} },500) }
      })
      bot.once('spawn', ()=>{
        this.log(id,'ok','Spawned')
        if(cfg.joinCmd){ this.log(id,'info','join '+cfg.joinCmd); bot.chat(cfg.joinCmd) }
        this._anti(s)
      })
      ...
      bot.on('end', ()=>{
        ...
        if(cfg.autoReconnect && !cfg.loginOnly){
          const d = Number(cfg.reconnectDelayMs||10000)
          this.log(id,'info','Reconnecting in '+Math.round(d/1000)+'s')
          s.reTimer=setTimeout(()=>{ set('reconnecting'); go() }, d)
        } else {
  ...
  stop(id){
    ...
    if(s.reTimer) clearTimeout(s.reTimer)
...
function guard(req,res,next){
  ...
  const hdr = normTok((req.headers.authorization||'').replace(/^Bearer\s+/i,''))
...
app.get('/api/debug/auth-check', (req,res)=>{
  ...
  const cookie = (req.headers.cookie||'').split(';').map(s=>s.trim()).find(s=>/^admin_token=/.test(s))
  const cookieVal = cookie ? normTok(cookie.split('=',2)[1]) : ''
  const ok = [hdr,q,cookieVal].some(v => v && v === ADMIN_TOKEN) || !ADMIN_TOKEN
  res.json({ok, tokenSet: !!ADMIN_TOKEN})
})
app.post('/api/set-token', (req,res)=>{
  const t = normTok((req.body && req.body.token) || '')
  if(!t) return res.status(400).json({error:'token required'})
  res.setHeader('Set-Cookie',`admin_token=${t}; Path=/; HttpOnly; SameSite=Lax`)
  res.json({ok:true})
})
...
app.post('/api/sessions', guard, async (req,res)=>{
  ...
  const cfg = {
    ...
    autoReconnect: Boolean(c.autoReconnect ?? true),
    reconnectDelayMs: Number(c.reconnectDelayMs||10000),
    loginOnly: Boolean(c.loginOnly ?? false)
  }
  try{
    const {id}=await M.start(cfg)
    res.json({id})
  }catch(e){
    res.status(500).json({error:e?.message||String(e)})
  }
})
app.post('/api/sessions/:id/stop', guard, (req,res)=>{
  try{ M.stop(req.params.id); res.json({ok:true}) }
...
app.post('/api/sessions/:id/chat', guard, (req,res)=>{
  ...
  try{ s.bot.chat(text.trim()); M.log(s.id,'you',text.trim()); res.json({ok:true}) }
  catch(e){ res.status(500).json({error:e?.message||String(e)}) }
})
app.post('/api/sessions/stop-all', guard, (req,res)=>{
  for(const id of [...M.sessions.keys()]){
    try{ M.stop(id) }catch{}
  }
  res.json({ok:true})
})
...
wss.on('connection', (ws, req)=>{
  if(ADMIN_TOKEN){
    ...
    const tok = normTok(q.token || '')
    if(tok !== ADMIN_TOKEN){ ws.close(); return }
  }
...
server.listen(PORT, ()=>console.log('AFK Console PRO v1.3.6-2 on '+PORT))
