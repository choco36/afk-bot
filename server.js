...
class BotManager {
  ...
  onBroadcast(fn){ this.listeners.add(fn); return ()=>this.listeners.delete(fn) }
  broadcast(type, payload){ for(const fn of this.listeners){ try{ fn({type,payload}) }catch{}} }
  list(){ return [...this.sessions.values()].map(s => ({
    ...
    createdAt: s.createdAt, lastEvent: s.lastEvent, afkMode: s.config.afkMode || 'jitter'
  }))}
  ...
  _startAntiAfk(session){
    ...
    const tick = ()=>{
      try{
        ...
        if(mode==='jitter'){ bot.setControlState('jump',true); setTimeout(()=>bot.setControlState('jump',false),250); if(Math.random()<.5) bot.swingArm('right') }
        else if(mode==='circle'){ bot.setControlState('forward', true); setTimeout(()=>bot.setControlState('forward', false), 1200) }
        else if(mode==='strafe'){ const side = Math.random()<.5 ? 'left':'right'; bot.setControlState(side, true); setTimeout(()=>bot.setControlState(side, false), 700) }
        else if(mode==='walkabout'){ const dir = Math.random()<.5 ? 'forward':'back'; bot.setControlState(dir, true); setTimeout(()=>bot.setControlState(dir, false), 700) }
        this.log(session.id, 'trace', `Anti-AFK tick (${mode})`)
      }catch(e){ this.log(session.id,'warn','Anti-AFK error: '+(e?.message||e)) }
    }
    session.antiAfkTimer = setInterval(tick, interval)
  }
  async start(config){
    const id = uuidv4()
    const s = { id, config, status:'starting', logs:[], createdAt:Date.now(), lastEvent:'init', bot:null, antiAfkTimer:null, reconnectTimer:null }
    this.sessions.set(id, s); this.broadcast('sessions', this.list())
    const startBot = ()=>{
      ...
      bot.once('login', ()=>{ set('online'); this.log(id,'ok', `Logged in as ${bot.username}`)})
      bot.once('spawn', ()=>{ this.log(id,'ok','Spawned'); if(config.joinCmd){ this.log(id,'info','join '+config.joinCmd); bot.chat(config.joinCmd) } this._startAntiAfk(s) })
      bot.on('message', m=> this.log(id,'chat', m?.toString?m.toString():String(m)))
      bot.on('kicked', r=>{ set('kicked'); this.log(id,'warn','KICKED: '+r) })
      bot.on('end', ()=>{ set('ended'); this.log(id,'info','Disconnected'); if(s.antiAfkTimer){ clearInterval(s.antiAfkTimer); s.antiAfkTimer=null } if(config.autoReconnect){ const d = Number(config.reconnectDelayMs||10000); this.log(id,'info','Reconnecting in '+Math.round(d/1000)+'s'); s.reconnectTimer=setTimeout(()=>{ set('reconnecting'); startBot() }, d) } })
      bot.on('error', e=> this.log(id,'error', e?.message||String(e)))
    }
    startBot()
    this.log(id,'hint','If Microsoft auth: check server logs for a device-code link.')
    return { id }
  }
  stop(id){
    ...
    s.status='stopped'; s.lastEvent='stopped'; this.log(id,'info','Session stopped'); this.broadcast('sessions', this.list())
  }
  stopAll(){ for(const id of [...this.sessions.keys()]){ try{ this.stop(id) }catch{} } return { ok:true } }
...
app.post('/api/sessions', assertAuth, async (req,res)=>{
  ...
  if(c.auth==='offline' && !c.username) return res.status(400).json({ error:'username is required in offline mode' })
  try{
...
app.post('/api/sessions/stop-all', assertAuth, (req,res)=> res.json(manager.stopAll()))
app.get('/api/sessions/:id/logs', assertAuth, (req,res)=> res.json(manager.logs(req.params.id)))
...
server.listen(PORT, ()=>{
  console.log('AFK Console PRO (HE) listening on http://localhost:'+PORT)
