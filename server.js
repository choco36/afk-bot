...
function guard(req,res,next){
  ...
  const cookieVal=cookie?cookie.split('=',2)[1]:''
  const ok=(hdr===`Bearer ${ADMIN_TOKEN}`)||(q===ADMIN_TOKEN)||(cookieVal===ADMIN_TOKEN)
  if(ok) return next(); return res.status(401).json({error:'unauthorized'})
}
