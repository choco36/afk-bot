...
// prismarine-auth is CJS – import default and destructure:
...
function sendToUid(uid, msg){
  const set = wsByUid.get(uid); if(!set) return
...
// ---- Microsoft LOGIN ONLY (device code flow) ----
    ...
    try{
      const flow = new Authflow(null, cacheDir, {
        ...
        onMsaCode: (data)=>{
          const uri = data.verification_uri || 'https://www.microsoft.com/link'
          const code = data.user_code || ''
          console.log('[MS-CODE]', uid, uri, code)
          sendToUid(uid, { type:'ms_code', payload:{ uri, code } })
        }
      })
      ...
      console.error('ms_start error', e)
...
server.listen(PORT,()=>console.log('AFK Console PRO v1.3.7-3 (no-token, ms-login) on '+PORT))
