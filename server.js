...
// persistent token cache per-browser (by uid cookie)
const TOKENS_DIR = path.join(__dirname, 'tokens')
import fs from 'fs'
if(!fs.existsSync(TOKENS_DIR)) fs.mkdirSync(TOKENS_DIR)

// per-connection ws registry by uid
  ...
  async start(cfg, uid){
    ...
    const go=()=>{
      ...
      const bot=mineflayer.createBot({
        ...
        profilesFolder, // reuse cached microsoft tokens
        onMsaCode: (data)=>{ try{
...
app.post('/api/ms/start', async (req,res)=>{
  ...
  // run the auth flow asynchronously; code is emitted through WS
  ;(async()=>{
    try{
      ...
      await flow.getMinecraftJavaToken() // waits until user finishes device-code login
...
server.listen(PORT,()=>console.log('AFK Console PRO v1.3.7-1 (no-token, ms-login) on '+PORT))
