...
// status: reflect server-side auth truthfully
app.get('/api/me',(req,res)=>{
  const hdr = normTok((req.headers.authorization||'').replace(/^Bearer\s+/i,''))
  const q   = normTok((req.query && req.query.token) || '')
  const cookie = (req.headers.cookie||'').split(';').map(s=>s.trim()).find(s=>/^admin_token=/.test(s))
  const cookieVal = cookie ? normTok(cookie.split('=',2)[1]) : ''
  const authorized = [hdr,q,cookieVal].some(v => v && v === ADMIN_TOKEN) || !ADMIN_TOKEN
  res.json({ tokenRequired: !!ADMIN_TOKEN, authorized })
})
...
server.listen(PORT,()=>console.log('AFK Console PRO v1.3.6-4 on '+PORT))
